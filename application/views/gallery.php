<div id="magnet_feed"> 
	<!--LAYOUT 1 START-->
	<div id="gallery_layout_1" class="layout1 gallery_layout row">
		<div class="gallery_col gallery_col_1 span6">
		    <div class="row circle_container flex_margin_bottom">
			    
				<div class="circle_creator_area"><span class="circle_creator"></span> created a</div>
				<div class="circle_of_strenth">Circle of Strength</div>
				<div class="circle_divider"></div>
				<div class="circle_area_wrapper">
					<div class="circle_area">
						<img src="<?php echo base_url(); ?>img/assets/dotted-circle-small.png"/>
						<div class="circle_goal"></div>
					</div>
				</div>
				<div class='share_circle'>
                	<div class='all_cap'>share</div>
                	<div class='circle_fb_share_btn facebook_share_btn'><img src='<?php echo base_url(); ?>img/icons/facebook.png'/></div>
					<div class='circle_tw_share_btn twitter_share_btn'><img src='<?php echo base_url(); ?>img/icons/twitter.png'/></div>
				</div>
				<div class="circle_rollover item_rollover">
					<div class="rollover_circle_content">
						<div class="view_circle_btn pink_btn all_cap">view</div>
						<div>Create your own Circle of Strength.<br/>Choose the <a>same goal</a> or <a>create a new one.</a></div>
					</div>
				</div>
				<div class="gallery_item_btn"></div>
			</div>
			
			<div class="row">
				<div class="span3 photo_container_1 photo_container"></div>
				<div class="span3 photo_container_2 photo_container"></div>	
			</div>
		</div>
		<div class="gallery_col gallery_col_2 span6">
			<div class="row">
				<div class="span3 photo_container_1 photo_container flex_margin_bottom"></div>
				<div class="span3 photo_container_2 photo_container flex_margin_bottom"></div>
			</div>
			<div class="row circle_container">
				
					<div class="circle_creator_area"><span class="circle_creator"></span> created a</div>
					<div  class="circle_of_strenth">Circle of Strength</div>
					<div class="circle_divider"></div>
					<div class="circle_area_wrapper">
						<div class="circle_area">
							<img src="<?php echo base_url(); ?>img/assets/dotted-circle-small.png"/>
							<div class="circle_goal"></div>
						</div>
					</div>
					<div class='share_circle'>
	                	<div class='all_cap'>share</div>
	                	<div class='circle_fb_share_btn facebook_share_btn'><img src='<?php echo base_url(); ?>img/icons/facebook.png'/></div>
						<div class='circle_tw_share_btn twitter_share_btn'><img src='<?php echo base_url(); ?>img/icons/twitter.png'/></div>
					</div>
					<div class="circle_rollover item_rollover">
						<div class="rollover_circle_content">
							<div class="view_circle_btn pink_btn all_cap">view</div>
							<div>Create your own Circle of Strength.<br/>Choose the <a>same goal</a> or <a>create a new one.</a></div>
						</div>
					</div>
					<div class="gallery_item_btn"></div>
			</div>
				
		</div>
	</div>
	<!--LAYOUT 1 END-->

	<!--LAYOUT 2 START-->
	<div id="gallery_layout_2" class="layout2 gallery_layout row">
		<div class="photo_col span3">
				<div class="span3 photo_container_1 photo_container flex_margin_bottom"></div>
				<div class="span3 photo_container_2 photo_container"></div>
		</div>
		<div class="circle_col span6">
			<div class="row circle_container">
				
					<div class="circle_creator_area"><span class="circle_creator"></span> created a</div>
					<div class="circle_of_strenth">Circle of Strength</div>
					<div class="circle_divider"></div>
					<div class="circle_area_wrapper">
						<div class="circle_area">
							<img src="<?php echo base_url(); ?>img/assets/dotted-circle-small.png"/>
							<div class="circle_goal"></div>
						</div>
					</div>
					<div class='share_circle'>
	                	<div class='all_cap'>share</div>
	                	<div class='circle_fb_share_btn facebook_share_btn'><img src='<?php echo base_url(); ?>img/icons/facebook.png'/></div>
						<div class='circle_tw_share_btn twitter_share_btn'><img src='<?php echo base_url(); ?>img/icons/twitter.png'/></div>
					</div>
					<div class="circle_rollover item_rollover">
						<div class="rollover_circle_content">
							<div class="view_circle_btn pink_btn all_cap">view</div>
							<div>Create your own Circle of Strength.<br/>Choose the <a>same goal</a> or <a>create a new one.</a></div>
						</div>
					</div>
					<div class="gallery_item_btn"></div>
			</div>
				
		</div>
		<div class="photo_col span3">
				<div class="span3 photo_container_1 photo_container flex_margin_bottom"></div>
				<div class="span3 photo_container_2 photo_container"></div>
		</div>
	</div>
	<!--LAYOUT 2 END-->
</div>

